...
def visualize(target, source, dist, instr):
    top, mid, bot = 0, 1, 2    
    arr = [ [0 for j in range(max(len(target), len(source)) * 2)] for i in range(3) ]

    i = len(target)
    j = len(source)

    # count of letters in alignment
    # used later for printing
    count = 0
    t = (max(len(target), len(source))*2) - 1
    while ((i!=0 and j!=-1) or (i!=-1 and j!=0)):
        # instruction for the current cell
        inst = instr[i][j]
...
# prints alignment array in the required format 
